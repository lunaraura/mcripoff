...
function BerryService:applyBerryEffect(player, itemDef)
	...
	elseif itemDef.type == "revive" then
		if self:hasAnyDefeatedSlot(player) then
			...
			self.worldService:pushEventLog(player, "Used Revive Berry (party revived)", "#9fd3ff")
			return true
		end
...
function BerryService:tryUseBerry(player, kind)
	...
	return self:applyBerryEffect(player, itemDef)
end
