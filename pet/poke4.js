...
class Brain {
    ...
    thinkPet(world) {
        ...
        if (isActive && player.activeControlMode === "manual") {
            this.thinkManualActivePet(world, h, followAnchor);
            return;
        }
    ...
    thinkManualActivePet(world, h, followAnchor) {
        if (h.manualCastRequest) {
            const handled = this.executeManualCast(world, h);
            if (handled) return;
        }

        if (h.command?.type) {
            const done = this.executeCommand(world, h, h.command, true, followAnchor);
            if (!done) return;
        }

        this.moveTowardAnchor(h, followAnchor, 18);
    }
...
class SpawnField {
    ...
    trySpawn(world) {
        ...
        const player = world.player;
        const level = world.getPlayerLevel() - 5;
        for (let i = 0; i < 14; i++) {
            ...
            const wild = world.factory.create(speciesKey, 1, x, z, { mode: "wild" , level: Math.max(1, level)});
...
class PlayerEntity {
    constructor(x, z) {
        ...
        this.selectedItemKey = this.itemBar[0];
    ...
    update(dt, input, world) {
        ...
        if (input.consumePress("KeyM")) {
            ...
            this.lastLog = `Active mode: ${this.activeControlMode}`;
        }
...
class World {
    ...
    initialize(starterSpeciesKey = "dog") {
        ...
        ownedStarter.moveset.push("pebbleshot", 'zap');
